...
export default function ChatWidgetN8n() {
  useEffect(() => {
    ...
    // Create the host div and attach shadow DOM
    ...
    // Add your styles (unchanged from your original design)
    ...
    // Create root div and inner HTML (unchanged)
    ...
    toggleBtn.addEventListener("click", () => {
      ...
      if (!panel.classList.contains("lw-hidden")) {
        shadow.getElementById("lw-input").focus();
      }
    });
    ...
    script.textContent = `
      ...
      createChat({
        container: document.querySelector("#lw-chat-widget-host").shadowRoot.querySelector("#lw-scroll"),
        input: document.querySelector("#lw-chat-widget-host").shadowRoot.querySelector("#lw-input"),
        sendButton: document.querySelector("#lw-chat-widget-host").shadowRoot.querySelector("#lw-send"),
