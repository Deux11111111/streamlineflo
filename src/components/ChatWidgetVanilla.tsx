...
export default function ChatWidgetVanilla() {
  useEffect(() => {
    ...
    const toggleBtn = shadow.getElementById("lw-toggle") as HTMLButtonElement;
    const panel = shadow.getElementById("lw-panel") as HTMLDivElement;
    const input = shadow.getElementById("lw-input") as HTMLInputElement;
    const sendBtn = shadow.getElementById("lw-send") as HTMLButtonElement;
    const scroll = shadow.getElementById("lw-scroll") as HTMLDivElement;
    const form = shadow.getElementById("lw-form") as HTMLFormElement;
    ...
    async function sendMessage(text: string) {
      ...
      try {
        ...
        try {
          console.log("[ChatWidgetVanilla] Trying no-cors fallback");
          ...
          // With no-cors, we can't read the response, so just assume it worked
          addMessage("assistant", "Message sent (no-cors mode). Check your n8n workflow to see if it received the message.");
          
        } catch (noCorsErr) {
          console.error("[ChatWidgetVanilla] No-cors fallback also failed", noCorsErr);
          addMessage("assistant", "Sorry, there was a connection error. Please check if your n8n webhook URL is correct and allows CORS.");
        }
      } finally {
