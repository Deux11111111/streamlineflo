    ...
    const scriptContent = `
      ...
      toggleBtn.addEventListener("click", async () => {
        ...
        if (!panel.classList.contains("lw-hidden")) {
          ...
          // Load previous session on first open (like n8n does)
          if (isFirstOpen) {
            isFirstOpen = false;
            try {
              ...
              if (response.ok) {
                ...
                // Handle any previous messages if returned
      ...
      async function sendMessage(text) {
        ...
        try {
          // Use EXACT n8n format: action as query param, chatInput and sessionId in body
          ...
          // Handle n8n response format - check for 'output' or 'text' field
          let botResponse = "";
          if (data.output) {
            ...
            // If it's an object, try to get the response from common fields
